...
TEST_CASE("every effect at its transparent settings passes the signal bit-exact")
{
    ...
    SUBCASE("parametric eq with every band off")
    {
        nullThrough(makeBuiltinEffect("incdaw.eqp", 48000.0));
    }

    SUBCASE("convolver at mix zero")
    {
    ...
    SUBCASE("beat gate at mix zero")
    {
        nullThrough(makeBuiltinEffect("incdaw.beatgate", 48000.0));
    }
