...
struct AppSettings {
    ...
    /// The browser's libraries and starred files (app/BrowserModel.h). Stored
    /// as strings rather than paths because that is what the file holds, and
    /// because a library on a volume that is not mounted must survive being
    /// read back — it is a preference, not a claim that the folder exists.
    struct Library {
        std::vector<std::string> roots;
        std::vector<std::string> favourites;

        /// Whether the first-run libraries have been offered.
        ///
        /// Without it, "no libraries" and "the user removed every library"
        /// look identical, and the defaults come back every launch against
        /// the user's explicit choice.
        bool seeded = false;
    } browser;

    /// Newest first, de-duplicated, bounded. Paths are stored as written; a
    /// file that has since moved is dropped when the menu is built, not here,
    /// because a disconnected volume is not the same as a deleted project.
    std::vector<std::string> recentProjects;

    static constexpr std::size_t maximumRecentProjects = 10;

    /// Moves `path` to the front, removing any earlier mention of it.
