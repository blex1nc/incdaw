...
| P9 | Browser pane: file browsing, search, favourites, preview, drag into project | **PARTS 1–2 DONE** — `app::Browser` (classification, folders-first listings, capped recursive search, favourites, recents, stage-and-rename persistence) and the pane itself (`INCDAWBrowserView`, leftmost, ⌘B, context menu, double-click opens projects and imports MIDI). Remaining: part 3 preview (an audition path in the engine), part 4 drag into the rack and the playlist |
